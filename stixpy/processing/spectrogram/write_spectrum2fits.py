...
def make_stix_header(spec, hdr = None, primary = False, respfile=None,extname=''):
    ...
    if primary:
        hdr.set('PARENT', spec.filename, "Parent Observation Data File")
...
def make_rate_header(rate_header,spec,respfile=None):
    ...
    hdr.set('CLOCKCOR', 'T', 'Clock Correction to UT') #should it be 1
    hdr.set('POISSERR','F','Poission Error') #sholud it be 0?
